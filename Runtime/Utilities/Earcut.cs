...
namespace CustomUIElements.Utilities
{
    namespace EarcutNet
    {
        public class Earcut
        {
            ...
            static void EarcutLinked(Node ear, IList<int> triangles, double minX, double minY, double invSize,
                int pass = 0)
            {
                if (ear == null)
                {
            ...
            static Node FindHoleBridge(Node hole, Node outerNode)
            {
                ...
                while (p != stop)
                {
                    if (hx >= p.x && p.x >= mx && hx != p.x && PointInTriangle(hy < my ? hx : qx, hy, mx, my,
            ...
            static bool PointInTriangle(double ax, double ay, double bx, double by, double cx, double cy, double px,
                double py)
            {
