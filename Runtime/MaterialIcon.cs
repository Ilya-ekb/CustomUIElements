...
{
    [UxmlElement]
    public partial class MaterialIcon : Label
    {
        // The name of the icon to be rendered.
       [UxmlAttribute("icon")] public string Icon
        {
            get => icon;
            set
            {
        ...
        private const string codepointsString = @"
10k e951
10mp e952
11mp e953
123 eb8d
...
";
        ...
        private void SetText()
        {
            ...
            {
                var code = GetCodepointByIconName(icon);
                if(string.IsNullOrEmpty(code))
                    text = code;
                else
                {
                    var unicode = int.Parse(code, System.Globalization.NumberStyles.HexNumber);
        ...
        private static string GetCodepointByIconName(string iconName)
        {
            ...
            if (iconNameToCodepoint != null && iconNameToCodepoint.Count != 0)
                return iconNameToCodepoint.GetValueOrDefault(iconName, iconName);
            
            iconNameToCodepoint = new Dictionary<string, string>();
            var codepointLines = codepointsString.Split("\n");
            foreach (var codepointLine in codepointLines)
            {
                var iconNameAndCodepoint = codepointLine.Trim().Split(" ");
                if (iconNameAndCodepoint.Length != 2) continue;
                var iN = iconNameAndCodepoint[0];
                var cp = iconNameAndCodepoint[1];
                iconNameToCodepoint[iN] = cp;
            }
